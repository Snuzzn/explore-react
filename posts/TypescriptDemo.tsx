...
const searchMovie = async (query: string) => {
  ...
  const res = await fetch(
    `https://www.omdbapi.com/?s=${query ? query : "s"}&apikey=${apiKey}`
  );
...
function TypescriptDemo() {
  ...
  React.useEffect(() => {
    ...
    const fetchMovieById = async () => {
      const res = await fetch(
        `https://www.omdbapi.com/?i=${defaultMovies[randMov]}&apikey=${apiKey}`
      );
      const data = await res.json();
      setMovie({
        title: data.Title,
  ...
  const renderContent = () => {
    if (error !== "")
      return (
        <ErrorCont>
          <img src={"/images/noMovie.svg"} alt="" width="200px" />
          <p>{error}</p>
        </ErrorCont>
      );
    else if (movie === null)
